...
import org.eweb4j.spiderman.xml.Parser;
import org.eweb4j.spiderman.xml.Parsers;
import org.eweb4j.spiderman.xml.Target;
import org.eweb4j.spiderman.xml.util.Attrs;
...
public class DefaultModelParser extends DefaultHandler implements ModelParser{
	...
	public DefaultModelParser(FetchRequest request, Target target, SpiderListener listener) {
		init(request, target, listener);
	}
	
	public List<Map<String, Object>> parse(Page page) throws Exception {
		String contentType = this.target.getModel()==null?"":this.target.getModel().getCType();
		...
		String isForceUseXmlParser = this.target.getModel()==null?"":this.target.getModel().getIsForceUseXmlParser();
		...
		TagNode rootNode = cleaner.clean(page.getContent().toString());
	...
	private Map<String, Object> parseJsonMap(Map item, final List<Field> fields){
		...
		for (Field field : fields){
			...
			String isNumber = field.getIsNumber();
...
//				String attribute = parser.getAttribute();
				...
				try {
					...
					//正则
					parseByRegex(regex, skipRgxFail, isNumber , newValues);
					// EXP表达式
					fel.getContext().set("$this", item);
					parseByExp(exp, isNumber , newValues);
	...
	private List<Map<String, Object>> parseXml(Page page, boolean isFromHtml) throws Exception{
        ...
        String validXml = ParserUtil.checkUnicodeString(page.getContent().toString());
				...
				try {
					if (xpath != null && xpath.trim().length() > 0) {
						...
						if (attribute != null && attribute.trim().length() > 0){
							...
							//正则
							parseByRegex(regex, skipRgxFail, isNumber,values);
							// EXP表达式
							parseByExp(exp, isNumber,values);
...
//		cleaner.getProperties().setTreatUnknownTagsAsContent(true);
		String isIgCom = this.target.getModel()==null?"":this.target.getModel().getIsIgnoreComments();
		if ("1".equals(isIgCom) || "true".equals(isIgCom))
			//忽略注释
			cleaner.getProperties().setOmitComments(true);
		cleaner.getProperties().setTreatDeprecatedTagsAsContent(true);
        ...
        final List<Field> fields = target.getModel()==null?new ArrayList<Field>():target.getModel().getField();
		String isModelArray = target.getModel()==null?"":target.getModel().getIsArray();
		String modelXpath = target.getModel()==null?"":target.getModel().getXpath();
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
	...
	private Map<String, Object> parseHtmlMap(Object item, final List<Field> fields){
		...
		for (Field field : fields){
			...
			for (int i = 0; i < parserList.size(); i++) {
				Parser parser = parserList.get(i);
				String skipErr = parser.getSkipErr();
				String xpath =DefaultLinkNormalizer.normalizeXml(parser.getXpath());
				String attribute = parser.getAttribute();
				String exp = DefaultLinkNormalizer.normalizeXml(parser.getExp());
				String regex = DefaultLinkNormalizer.normalizeXml(parser.getRegex());
				String skipRgxFail = parser.getSkipRgxFail();
				try {
					if (xpath != null && xpath.trim().length() > 0) {
						...
						if (attribute != null && attribute.trim().length() > 0){
							for (Object nodeVal : nodeVals){
								...
								values.add(attrVal);
								if("1".equals(isNumber) || "true".equals(isNumber))
								{
									parseToNumber(values);
								}
							}
							//正则
							parseByRegex(regex, skipRgxFail, isNumber , values);
	...
	private void parseByExp(String exp, String isNumber, Collection<Object> list) {
	...
	private void parseByRegex(String regex, String skipRgxFail, String isNumber,Collection<Object> list) {
		if (regex == null || regex.trim().length() == 0)
			return ;
		List<Object> newVals = new ArrayList<Object>(list.size());
		if(list.size() == 0)
		{
			String page_content = fel.eval("$page_content").toString();
			List<String> vals = CommonUtil.findByRegex(page_content, regex);
			//如果REGEX找不到
			if (vals == null) {
		...
		if (!newVals.isEmpty()){
			...
			if("1".equals(isNumber) || "true".equals(isNumber))
			{
				parseToNumber(newVals);
			}
			list.addAll(newVals);
		}
	}
	public void parseToNumber(List<Object> datas)
	{
...
//        }
		...
		HtmlCleaner cleaner = new HtmlCleaner();
		cleaner.getProperties().setTreatDeprecatedTagsAsContent(true);
		TagNode tagNode = cleaner.clean(new File("e:\\test3.html"));
		/*String xml = ParserUtil.xml(tagNode, true);
		System.out.println(xml);*/
		Object[] nodes = tagNode.evaluateXPath("//head | //body");
		for (Object n : nodes){
