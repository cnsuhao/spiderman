...
public class TaskPushPointImpl implements TaskPushPoint{
	...
	public synchronized FetchResult pushTask(FetchRequest request,FetchResult result) throws Exception{
		
		Collection<Task> successTasks = new ArrayList<Task>();
			...
			try{
				...
				if (vhs == null || vhs.getValidHost() == null || vhs.getValidHost().isEmpty()){
					if (!CommonUtil.isSameHost(task.site.getUrl(), task.url)) {
						listener.onInfo(Thread.currentThread(), request, "task.url->"+task.url+"'s host is not the same as site.host->" + task.site.getUrl());
						continue;
					}
				}else{
					boolean isOk = false;
					...
					if (!isOk) {
					    listener.onInfo(Thread.currentThread(), request, "task.url->"+task.url+"'s host is not in the validHosts");
						continue;
					}
				}
...
//					System.out.println("push task->"+task+" push the queue ... result -> " + isOk);
		...
		result.setValidTasks(successTasks);
		return result;
	}
